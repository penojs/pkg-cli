...
module.exports = async (pkgName, options = {}) => {
  ...
  try {
    ...
    await installDependencies(pkgName, options)
  } catch (e) {
    logError(e)
    process.exit(1)
  }
...
async function chooseOneTemplate(options) {
  ...
  await inquirer
    .prompt([
      ...
      {
        type: 'confirm',
        message: 'Do you want install dependency auto?',
        name: 'autoInstall',
        default: true,
      },
    ])
    .then(answers => {
      ...
      options.autoInstall = answers.autoInstall
    })
}
...
async function installDependencies(pkgName, options) {
